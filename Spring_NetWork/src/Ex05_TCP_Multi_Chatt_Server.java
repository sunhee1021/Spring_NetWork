...
//다중 채팅
//서버 1개
//여러명의 클라이언트 접속 (채팅방이 1개임) _ HashMap으로 만들겠다 > key(사용자ID, socket or DataOutputStream or DataInputStream)...
																//소켓을써도되고, 아웃풋 , 인풋 써도 되고~
//클라이언트 1명( socket )
public class Ex05_TCP_Multi_Chatt_Server {
	...
	//DataOutputStream 각각의 socket의 통로 주소값을 가지고 있는거임
	...
	//1. 서버 초기화 작업
	public void init() {  //생성자랑은 다른거임, 명시적으로 호출되야하는 함수
		try {
	...
	//2. 접속된 모든 클라이언트(여기서 말하는 클라이언트는 socket)에게 (socket) 메시지를 전달하는 함수
	//Map(key ,value)
